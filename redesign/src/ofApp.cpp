...
//    ofSetColor(248,232,160); //JOY color
    ...
    ofSetColor(70, 74, 81);
    mediumfont.drawString(eventString, 870, 715);  //type JOY..
    ...
    for (int i = 0; i < 20; i++) {
        ...
        if(angerOn){
           ...
           x = redradius * noise_value * cos(deg * DEG_TO_RAD);
           y = redradius * noise_value * sin(deg * DEG_TO_RAD);
...
void ofApp::keyPressed(int key){
    ...
    if (key == '1') {
        eventString = "Joy";
        change.set(248,232,160);   //color change
        beeb.play();
